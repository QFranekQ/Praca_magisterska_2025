...
def ekstrahuj_cechy_z_filmu(sciezka_filmu):
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            # Zapisujemy pozycję lewej i prawej dłoni (lub 0 jeśli nie ma)
            left = right = None
            ...
            # Dodajemy obie dłonie – jeśli którejś brak, to zerowy wektor
            if left is None:
                left = np.zeros((21, 3))
            if right is None:
                right = np.zeros((21, 3))
...
sciezka_docelowa = "sekwencje_skalowanie_norm"
